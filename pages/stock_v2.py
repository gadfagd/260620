...
with st.sidebar:
    ...
    st.caption("학습 기간·예측 기간은 각 탭 상단의 슬라이더에서 조절하세요.")
...
with tab_rank:
    ...
    learn_r = rc1.select_slider("학습 기간", options=LEARN_OPTIONS, value=24,
                                format_func=fmt_learn, key="learn_rank")
    pred_r = rc2.select_slider("예측 기간", options=PRED_OPTIONS, value=3,
                               format_func=fmt_pred, key="pred_rank")
...
with tab_detail:
    ...
    learn_d = dc1.select_slider("학습 기간", options=LEARN_OPTIONS, value=24,
                                format_func=fmt_learn, key="learn_detail")
    pred_d = dc2.select_slider("예측 기간", options=PRED_OPTIONS, value=3,
                               format_func=fmt_pred, key="pred_detail")
    ...
    if st.button("🔮 예측 실행", type="primary", key="detail_run"):
        ...
        fc_lstm = None
        if TF_AVAILABLE:
            with st.spinner("LSTM(딥러닝) 학습 중... (수십 초 걸릴 수 있어요)"):
        ...
        st.markdown(
            "<div style='display:flex; gap:1rem; flex-wrap:wrap; margin:0.3rem 0 0.6rem;'>"
            + "".join(cards) + "</div>",
            unsafe_allow_html=True,
        )

        if not TF_AVAILABLE:
            st.caption("⚠️ 이 환경에는 TensorFlow가 없어 선형회귀만 표시됩니다.")
        elif fc_lstm is None:
            st.caption("⚠️ 데이터가 부족해 LSTM 예측은 생략했습니다. 학습 기간을 늘려보세요.")
        ...
        fig.add_trace(go.Scatter(x=lin_fut["ds"], y=lin_fut["yhat"], name="선형회귀",
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color="rgba(0,0,0,0)"), name="선형회귀 구간",
            hoverinfo="skip", showlegend=False), row=1, col=1)

        if lstm_fut is not None:
            fig.add_trace(go.Scatter(x=lstm_fut["ds"], y=lstm_fut["yhat"], name="LSTM",
                                     line=dict(color="#7B5BD6", width=2)), row=1, col=1)
            fig.add_trace(go.Scatter(
                x=pd.concat([lstm_fut["ds"], lstm_fut["ds"][::-1]]),
                y=pd.concat([lstm_fut["yhat_upper"], lstm_fut["yhat_lower"][::-1]]),
                fill="toself", fillcolor="rgba(123,91,214,0.10)",
                line=dict(color="rgba(0,0,0,0)"), name="LSTM 구간",
                hoverinfo="skip", showlegend=False), row=1, col=1)
        ...
        with st.expander("📋 예측 데이터 비교"):
            comp = pd.DataFrame({
                "날짜": lin_fut["ds"].dt.strftime("%Y-%m-%d").values,
                "선형회귀": lin_fut["yhat"].round(2).values,
            })
            if lstm_fut is not None:
                comp["LSTM"] = lstm_fut["yhat"].round(2).values
            st.dataframe(comp.reset_index(drop=True),
                         use_container_width=True, hide_index=True)
    else:
        st.markdown("👆 위에서 기간을 정한 뒤 **예측 실행** 버튼을 누르면 선형회귀와 LSTM 결과가 함께 나옵니다.")
